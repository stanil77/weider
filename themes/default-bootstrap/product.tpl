					...
					<div class="content_prices clearfix">
						{if $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
							...
							<div class="price">
								...
								<p id="reduction_percent" {if $group_reduction==0}{if (!$product->specificPrice||$product->specificPrice.reduction_type != 'percentage')}style="display:none;"{/if}{/if}>
									<span id="reduction_percent_display">
										{if $product->specificPrice && $product->specificPrice.reduction_type == 'percentage'}-{$product->specificPrice.reduction*100}%{elseif $group_reduction!=0}-{$group_reduction*100}%{/if}
									</span>
								</p>
								<p id="reduction_amount" {if !$product->specificPrice || $product->specificPrice.reduction_type != 'amount' || $product->specificPrice.reduction|floatval ==0} style="display:none"{/if}>
									...
									{if $product->specificPrice && $product->specificPrice.reduction_type == 'amount' && $product->specificPrice.reduction|floatval !=0}
										спестявате:{convertPrice price=$productPriceWithoutReduction-$productPrice|floatval}
									{/if}
									</span>
								</p>
...
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
    ...
    *}
    ...
    {if $product->description}
			<!-- More info -->
            {assign var="s_hastable" value=strpos($product->description,"<p>stablica</p>")}
            {if $s_hastable!=false}
                {assign var="s_table" value = explode("<p>stablica</p>",$product->description)}
            {/if}
            <section class="page-product-box">
            ...
            {if $s_hastable!=false}
            <section class="page-product-box">
				<h3 class="page-product-heading">Хранителни стойности</h3>{/if}
               			<div  class="rte">
                            {if $s_hastable!=false}
                                {$s_table[1]}
                            {/if}
    ...
    {if (isset($product) && $product->description) || (isset($features) && $features) || (isset($accessories) && $accessories) || (isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB) || (isset($attachments) && $attachments) || isset($product) && $product->customizable}
        ...
        <div id="more_info_block" class="col-md-12 review-tab" style="padding-left:0px; padding-right:0px;">
            <ul id="more_info_tabs" class="idTabs idTabsShort nav nav-tabs">
                ...
                {if isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB}<li><a href="#idTab11">{l s='Product Reviews'}</a></li>{/if}
            </ul>
            
            <div id="more_info_sheets" class="sheets align_justify" style="color:white; margin-top:14px;">
                {if isset($product) && $product->description}
                    <!-- full description -->
...
{if ($product->specificPrice && $product->specificPrice.reduction && $product->specificPrice.reduction_type == 'percentage')}
    ...
    {if $group_reduction!=0}
        {addJsDef reduction_percent=$group_reduction*100|floatval}
    {else}    
