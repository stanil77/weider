...
class ImageResizer
{
	...
	public static function thumbnail($image, $cache_image, $size, $image_type = 'jpg', $disable_cache = false)
	{
		...
		if (!file_exists(_PS_TMP_IMG_DIR_.$cache_image))
		{
			...
			if (!ImageResizer::checkImageMemoryLimit($image))
				return false;
			...
			else
			{
				...
				ImageResizer::resize($image, _PS_TMP_IMG_DIR_.$cache_image, $ratio_x, $size, $image_type);
			}
		}
	...
	public static function resize($src_file, $dst_file, $dst_width = null, $dst_height = null, $file_type = 'jpg', $force_type = false)
	{
		...
		$src_image = ImageResizer::create($type, $src_file);
		...
		if (!ImageResizer::checkImageMemoryLimit($src_file))
			return false;
		...
		return (ImageResizer::write($file_type, $dest_image, $dst_file));
	}
	...
	public static function validateUpload($file, $max_file_size = 0)
	{
		...
		if (!ImageResizer::isRealImage($file['tmp_name'], $file['type']) || !ImageResizer::isCorrectImageFileExt($file['name']))
	...
	public static function cut($src_file, $dst_file, $dst_width = null, $dst_height = null, $file_type = 'jpg', $dst_x = 0, $dst_y = 0)
	{
		...
		$src = array(
			'width' => $src_info[0],
			'height' => $src_info[1],
			'ressource' => ImageResizer::create($src_info[2], $src_file),
		);
		...
		$dest['ressource'] = ImageResizer::createWhiteImage($dest['width'], $dest['height']);

		$white = imagecolorallocate($dest['ressource'], 255, 255, 255);
		imagecopyresampled($dest['ressource'], $src['ressource'], 0, 0, $dest['x'], $dest['y'], $dest['width'], $dest['height'], $dest['width'], $dest['height']);
		imagecolortransparent($dest['ressource'], $white);
		$return = ImageResizer::write($file_type, $dest['ressource'], $dst_file);
		return	$return;
	}
